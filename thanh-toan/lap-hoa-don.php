<?php
  ...
  if (isset($_SESSION['userId']) && isset($_POST['firstName']) && isset($_POST['lastName']) && isset($_POST['address']) && isset($_POST['email']) && isset($_POST['city']) && isset($_POST['district']) && isset($_POST['phone']) && isset($_POST['paymentMethod'])) {
    $sql = 'INSERT INTO `bill` (`user_id`, `date`, `status`, `pay`, `total`, `phone`, `province`, `district`, `address`) VALUES (?,?,?,?,?,?,?,?,?)';
    ...
    $stmt->bind_param('dsssdssss', $_SESSION['userId'], $date, $status, $pay, $total, $phone, $province, $district, $address);
    $stmt->execute();
